const PRODUCTION_SUPABASE_URL = 'https://zkphbzcomixukygigpka.supabase.co';
const PRODUCTION_SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InprcGhiemNvbWl4dWt5Z2lncGthIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTEyNzE1MiwiZXhwIjoyMDY2NzAzMTUyfQ.UL-6kZg73_fjBxWVg30Mc756PGs2wUNkIQs9aqRMlwU';

async function removeNoFollowFromAllPosts() {
  console.log('🚀 Fetching all blog posts from PRODUCTION...');

  // Fetch all blog posts
  const response = await fetch(`${PRODUCTION_SUPABASE_URL}/rest/v1/blog_posts?select=*`, {
    headers: {
  ...
  for (const post of posts) {
    try {
      ...
      if (contentModified) {
        const updateResponse = await fetch(`${PRODUCTION_SUPABASE_URL}/rest/v1/blog_posts?id=eq.${post.id}`, {
          method: 'PATCH',
          headers: {
            'apikey': PRODUCTION_SERVICE_KEY,
            'Authorization': `Bearer ${PRODUCTION_SERVICE_KEY}`,
